...
def displayHand(hand):
    ...
    for letter in hand.keys():
        for j in range(hand[letter]): #for loop, loops as many times as handL value;
...
def dealHand(n):
    ...
    for i in range(numVowels):
        x = VOWELS[random.randrange(0,len(VOWELS))]#random number, starts at 0, stops lenV
        hand[x] = hand.get(x, 0) + 1
        
    for i in range(numVowels, n): #start, at numV since already done, stops at n.    
...
def updateHand(hand, word):
    ...
    try:
        ...
        return False;
#all that's needed is to make sure the original hand dict isn't modded.
#
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    """
    global ScoreT; #keeps track of score
    Peer=False;
    while calculateHandlen(hand)>0:
        ...
        if word=='.':
            ...
            #Otherwise (the word is valid):
                ...
                ScoreT=ScoreT+InPScore;
                print ('"' + str(word) + '"' + ' earned ' + str(InPScore) +' points.' + ' Total: ' + str(ScoreT) + ' points');
                print
                # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                # Update the hand
                hand=updateHand(hand, word);
    if Peer==True:
        print ('Goodbye! Total score: ' + str(ScoreT));
        ScoreT=0;
    else:
        print('Run out of letters. Total score: ' + str(ScoreT)+ ' points.')
        ScoreT=0;
